    ...
    <div class="col-xl-12">
        ...
        <div class="card card-flush h-lg-100" id="kt_contacts_main">
            ...
            <div class="card-body pt-12">
                ...
                <!--begin::Form-->
                <form class="form" action="{{ url('/layanan/assignment_update/' . $data->id) }}" method="post"
                                ...
                                <select aria-label="Come From" data-control="select2" data-placeholder="Come From"
                                    ...
                                    @foreach ($marketing as $key => $item)
                                        <option value="{{ $item->id }}"
                                            {{ $data->marketing_id == $item->id ? 'selected' : '' }}>{{ $item->name }} -
                                            {{ $item->type }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="w-50">
                            ...
                            <div>
                                <select aria-label="Come From" data-control="select2" data-placeholder="Come From"
                                    ...
                                    @foreach ($nasabah as $key => $item1)
                                        <option value="{{ $item1->id }}"
                                            {{ $data_assignment->customer_id == $item1->id ? 'selected' : '' }}>
                                            {{ $item1->name }} - {{ $item1->type }} - {{ $item1->category }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <!--end::Select2-->
